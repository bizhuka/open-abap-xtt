...
CLASS lcl_named_node_map IMPLEMENTATION.
  METHOD if_ixml_named_node_map~get_item.
    " local-abap patch: index is zero-based, like SAP iXML
    READ TABLE mt_list INDEX index + 1 INTO rval.
    IF sy-subrc <> 0.
      WRITE: / `{ index } not found { lines( mt_list ) }`.
    ENDIF.
  ENDMETHOD.
...
CLASS lcl_node DEFINITION.
  PUBLIC SECTION.
    ...
    " local-abap patch: attributes are stored as lcl_node instances, so the
    " node must also be castable to if_ixml_attribute (pure alias interface);
    " same for text nodes (if_ixml_text, used via document->create_text)
...
CLASS lcl_node IMPLEMENTATION.
  ...
  METHOD if_ixml_node~insert_child.
    ...
    " If the child is already attached to another parent, remove it first
    ...
    " Insert into child list before ref_child (or append if ref_child is initial)
    IF mo_children->insert( ii_node = new_child
                            ii_ref  = ref_child ) = abap_true.
      rval = 0.
    ELSE.
      rval = 4.
    ENDIF.
  ...
  METHOD if_ixml_node~clone.
    " local-abap patch: deep copy of the node (name, prefix, value, attributes, children)
    ...
    DATA lo_attr_copy TYPE REF TO lcl_node.

    CREATE OBJECT lo_copy.
    lo_copy->if_ixml_node~set_name( mv_name ).
    lo_copy->if_ixml_node~set_namespace_prefix( mv_namespace ).
    lo_copy->if_ixml_node~set_value( mv_value ).

    li_iter = mi_attributes->create_iterator( ).
    DO.
      li_attr = li_iter->get_next( ).
      IF li_attr IS INITIAL.
        EXIT.
      ENDIF.
      CREATE OBJECT lo_attr_copy.
      lo_attr_copy->if_ixml_node~set_name( li_attr->get_name( ) ).
      lo_attr_copy->if_ixml_node~set_value( li_attr->get_value( ) ).
      lo_copy->if_ixml_node~get_attributes( )->set_named_item_ns( lo_attr_copy ).
    ENDDO.
    ...
    DO.
      ...
      IF li_child IS INITIAL.
        EXIT.
      ENDIF.
      lo_copy->if_ixml_element~append_child( li_child->clone( ) ).
    ENDDO.
  ...
  METHOD if_ixml_node~get_namespace_uri.
    " local-abap patch: the naive parser tracks prefixes only, URIs are not stored
    rval = ''.
  ENDMETHOD.

  METHOD if_ixml_element~get_attributes.
    " local-abap patch: same map as on node level
  ...
  METHOD if_ixml_element~clone.
    " local-abap patch: same deep copy as on node level
  ...
  METHOD if_ixml_element~create_iterator.
    " local-abap patch: iterate the whole subtree (self + descendants),
    " same breadth-first walk as find_from_name_ns above
    DATA li_iterator TYPE REF TO if_ixml_node_iterator.
    DATA li_node     TYPE REF TO if_ixml_node.
    ...
    LOOP AT lt_nodes INTO li_top.
      li_iterator = li_top->get_children( )->create_iterator( ).
      DO.
        li_node = li_iterator->get_next( ).
        IF li_node IS INITIAL.
          EXIT.
...
*        WRITE '@KERNEL console.dir(name);'.
  ...
  METHOD if_ixml_element~find_from_name.
    " local-abap patch: same lookup as the _ns variant (URIs are not tracked)
    val = if_ixml_element~find_from_name_ns( name  = name
                                             depth = depth ).
  ENDMETHOD.
  ...
  METHOD collect_elements_by_tag_name.
    ...
    DO.
      ...
      IF li_node->get_name( ) <> '#text'.
        ...
        " local-abap patch: when a URI is passed (get_elements_by_tag_name_NS)
        " it cannot be compared - this DOM only tracks prefixes. Match by name.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        ...
        document TYPE REF TO if_ixml_document.
  PRIVATE SECTION.
    " local-abap patch: attribute values are ANY chars except the quote ([^"]*).
    " The original whitelist broke on empty values (name="") and non-ASCII
    " content (font typeface="맑은 고딕" in xlsx themes) - JS \w is ASCII-only.
...
CLASS lcl_parser IMPLEMENTATION.
  ...
  METHOD if_ixml_parser~set_validating.
    " local-abap patch: validation is not supported, accept "no validation"
    " (co_no_validation = 0) as a no-op instead of failing on the todo-stub
...
* value
  ...
  METHOD parse_attributes.
    ...
    DO.
      ...
      " local-abap patch: strip the namespace prefix from attribute names
      " (r:id -> id), same as element handling - SAP iXML returns local names
      IF lv_name CA ':'.
        DATA lv_attr_prefix TYPE string.
        SPLIT lv_name AT ':' INTO lv_attr_prefix lv_name.
        li_node->set_namespace_prefix( lv_attr_prefix ).
      ENDIF.
